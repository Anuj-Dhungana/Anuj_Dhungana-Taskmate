...
export const getWorkspaceDMs = asyncHandler(async (req, res) => {
    const workspaceId = req.params.workspaceId || req.query.workspaceId;
    if (!workspaceId) {
        return res.status(400).json({ message: "workspaceId is required" });
    }
...
export const renameChannel = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { name } = req.body;
    if (!name?.trim()) {
...
export const addMembersToChannel = asyncHandler(async (req, res) => {
    const { id } = req.params;
